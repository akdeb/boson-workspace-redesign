/**
 * The SQLite schema, as a module rather than a file on disk.
 *
 * It used to be `readFileSync("lib/db/schema.sql")`, which works locally and cannot work on
 * a serverless host: the bundler traces imports, so a file nothing imports is not shipped.
 * `schema.postgres.sql` beside this is the Supabase twin — keep the two in step.
 */
